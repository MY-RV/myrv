...
export const World: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const svgMapDomEl = containerEl.querySelector("#map") as SVGSVGElement;
        const svgCountryDomEl = containerEl.querySelector("#country") as SVGSVGElement;
        const countryNameEl = containerEl.querySelector(".info span") as HTMLElement;

        // Si el SVG se encuentra fuera del DOM visible, quizá desees controlarlo (por ejemplo, con style.display)
        // En este ejemplo los dejamos en el DOM pero con display none

        // Variables de Three.js y parámetros
        ...
        let staticMapUri: string;
        const bBoxes: DOMRect[] = [];
        const dataUris: string[] = [];

        // Se obtienen los paths del mapa (asegúrate de que existan en el SVG)
        const svgCountries: SVGPathElement[] = svgMapDomEl
            ? Array.from(svgMapDomEl.querySelectorAll("path"))
            : [];

        // Función para actualizar la posición del puntero en coordenadas normalizadas
        ...
        // Creación de la geometría y mallas de la esfera
        ...
        // Carga y asigna una textura desde un URI al material
        function setMapTexture(material: THREE.Material, URI: string) {
            textureLoader.load(URI, (texture) => {
                texture.repeat.set(1, 1);
                // Se asigna la textura al material; se usa una coerción si es necesario
                (material as any).map = texture;
                material.needsUpdate = true;
            });
        }

        // Prepara las texturas en alta resolución para el globo
        ...
        // Prepara las texturas en baja resolución para la selección
        ...
        function updateMap(uv: { x: number; y: number }) {
            if (!svgMapDomEl) return;
            ...
            for (let i = 0; i < svgCountries.length; i++) {
                ...
                // Se verifica si el punto está dentro del bounding box
                if (
        ...
        // Función de renderizado (se llama en cada tick de gsap)
        function render() {
            controls.update();
        ...
        // Actualiza el tamaño del render
        function updateSize() {
        ...
        // Crea la interfaz de controles con lil-gui
        ...
        // Agregamos algunos _event listeners_
        ...
        // Inicializamos la escena y los controles
        initScene();
        createControls();

        // Función de _cleanup_ al desmontar el componente
        return () => {
            window.removeEventListener("resize", updateSize);
            containerEl.removeEventListener("touchstart", () => { });
            containerEl.removeEventListener("mousemove", () => { });
            containerEl.removeEventListener("click", () => { });
