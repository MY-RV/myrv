        ...
        <div className="mt-2 flex gap-4 cursor-pointer">
            {items != null && (
                items.map((item, index) => {
                    return item.url ? (
                        <Link target="_blank" className="text-[#787878]   opacity-60 hover:opacity-100 hover:transition-opacity flex mt-2" key={index} href={item.url}>
                            {item.icon && (
                            ...
                            <label className="text-[#787878] text-sm  cursor-pointer">{item.description}</label>
                        </Link>
                    ) : null;
                })
            )}
